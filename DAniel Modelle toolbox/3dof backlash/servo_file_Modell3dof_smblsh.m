function F = servo_file_Modell3dof_smblsh(Dx,x,path,parameters)
%x = [q1 q2 q3 dq1 dq2 dq3]
%parameters = [d_ax d_ax2];
%path = [x y dx dy d2x d2y]
   

F(1) = parameters(1)*cos(x(2))+parameters(2)*cos(x(2)+x(3))-path(1); %x constraint
F(2) = parameters(1)*sin(x(2))+parameters(2)*sin(x(2)+x(3))-path(2); %y constraint
